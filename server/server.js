...
app.patch('/api/services/:id',(req,res)=>{const data=load();const item=data.services.find(s=>s.id===req.params.id);if(!item)return res.status(404).json({error:'Service not found'});Object.assign(item,req.body);save(data);res.json(item)})
app.post('/api/inquiries',(req,res)=>{const data=load();const item={id:id(),name:req.body.name,email:req.body.email,message:req.body.message,createdAt:new Date().toISOString()};data.inquiries.unshift(item);save(data);res.status(201).json({success:true,item})})
