...
app = FastAPI(title="ClipForge AI", version="3.0.0", description="Local AI YouTube-to-Shorts generator with smart highlight ranking, captions and exports.")
...
HOOK_WORDS = {"amazing", "important", "secret", "mistake", "problem", "solution", "best", "worst", "never", "always", "how", "why", "truth", "tip", "tips", "learn", "learned", "money", "success", "failure", "hack", "easy", "hard", "avoid", "key", "reason", "idea", "powerful", "actually", "real", "wrong", "simple", "nobody", "everyone", "first", "only", "instead", "remember", "warning", "free", "shocking", "crazy", "story", "secret", "lesson"}
...
def build_candidates(transcription: dict[str, Any], duration: float, target: int, clip_duration: int) -> list[dict[str, Any]]:
    ...
    for i, seg in enumerate(segments):
        ...
        start = max(0.0, anchor_start - min(6.0, window * 0.2))
        if duration > window:
    ...
    selected = []
    for c in candidates:
        overlap = any(c["start"] < x["end"] and c["end"] > x["start"] for x in selected)
        if overlap:
            continue
        selected.append(c)
...
def make_metadata(text: str, number: int) -> dict[str, Any]:
    ...
    hashtags = ["#Shorts", "#ClipForgeAI"] + [f"#{re.sub(r'[^a-zA-Z0-9]', '', x).title()}" for x in topics[:5]]
    return {"title": make_title(text, number), "description": text[:500], "hashtags": list(dict.fromkeys(hashtags))}


def create_thumbnail(input_file: Path, output_file: Path, start: float) -> None:
    run_ffmpeg(["-y", "-ss", f"{start:.3f}", "-i", str(input_file), "-frames:v", "1", "-vf", "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920", str(output_file)])
...
def download_youtube(url: str, destination: Path, job_id: str) -> None:
    ...
    for selector in selectors:
        try:
            opts: dict[str, Any] = {"format": selector, "outtmpl": template, "merge_output_format": "mp4", "noplaylist": True, "ffmpeg_location": FFMPEG, "restrictfilenames": True, "quiet": False, "no_warnings": False, "overwrites": True}
            if runtime:
            ...
            last_error = str(exc)
            update_job(job_id, message="Trying a compatible YouTube format…")
...
def process_file(input_file: Path, job_id: str, options: ProcessOptions) -> dict[str, Any]:
    ...
    transcription = transcribe(input_file, options.model) if has_audio else {"text": "", "segments": [], "language": "unknown"}
    update_job(job_id, progress=62, message="Scoring hooks, context and retention signals…")
    ...
    for index, highlight in enumerate(highlights, 1):
        ...
        try:
            create_thumbnail(input_file, thumb, start)
        except Exception:
            thumb = None
    ...
    manifest = {"source_duration": round(duration, 2), "source_resolution": f"{width}x{height}", "clips": clips}
    ...
    if transcription.get("error"):
        result["warning"] = "Whisper could not transcribe this video, so fallback highlight selection was used."
    return result
...
def home() -> dict[str, str]:
    return {"status": "running", "service": "ClipForge AI", "version": "3.0.0"}


@app.get("/health")
def health() -> dict[str, Any]:
    return {"status": "healthy", "ffmpeg": Path(FFMPEG).exists(), "ffmpeg_path": FFMPEG, "whisper": module_available("whisper"), "yt_dlp": module_available("yt_dlp"), "deno": bool(youtube_runtime_args()), "max_clips": MAX_CLIPS}


@app.post("/process")
async def process_upload(background_tasks: BackgroundTasks, file: UploadFile = File(...), num_clips: int = 5, clip_duration: int = 30, captions: bool = True, model: str = "base", smart_crop: bool = True, generate_metadata: bool = True) -> dict[str, Any]:
    options = ProcessOptions(num_clips=num_clips, clip_duration=clip_duration, captions=captions, model=model, smart_crop=smart_crop, generate_metadata=generate_metadata)
    extension = Path(file.filename or "").suffix.lower()
    if extension not in ALLOWED_EXTENSIONS:
        raise HTTPException(400, "Unsupported video format. Use MP4, MOV, MKV, AVI, WebM or M4V.")
...
def download_clip(filename: str) -> FileResponse:
    ...
    media = "image/jpeg" if candidate.suffix.lower() == ".jpg" else "application/octet-stream" if candidate.suffix.lower() == ".json" else "video/mp4"
...
def download_all(job_id: str) -> FileResponse:
    ...
    if not files:
        raise HTTPException(404, "No generated clips found")
