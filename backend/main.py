...
app = FastAPI(title="ClipForge AI", version="5.0.0", description="Local AI video repurposing studio")
...
def video_filter(smart_crop:bool,crop_mode:str)->str:
    ...
    if crop_mode=="blur":
        return "split=2[bg][fg];[bg]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,boxblur=18:2[blur];[fg]scale=1080:1920:force_original_aspect_ratio=decrease[main];[blur][main]overlay=(W-w)/2:(H-h)/2"
...
def process_file(input_file:Path,job_id:str,options:ProcessOptions)->dict[str,Any]:
    ...
    manifest={"source_duration":round(duration,2),"source_resolution":f"{width}x{height}","language":transcription.get("language","unknown"),"transcript":transcription.get("text",""),"clips":clips,"settings":options.model_dump()}
    (OUTPUT_DIR/f"{job_id}_manifest.json").write_text(json.dumps(manifest,indent=2,ensure_ascii=False),encoding="utf-8")
    result={"status":"completed","job_id":job_id,"source_duration":round(duration,2),"source_resolution":f"{width}x{height}","has_audio":has_audio,"language":transcription.get("language","unknown"),"transcript":transcription.get("text",""),"number_of_clips":len(clips),"clips":clips,"settings":options.model_dump(),"download_all_url":f"/download-all/{job_id}","output_format":"9:16","output_resolution":"1080x1920"}
...
def run_job(job_id:str,input_file:Path,options:ProcessOptions)->None:
    try:
        update_job(job_id,status="processing",progress=32,message="Starting AI analysis…");result=process_file(input_file,job_id,options);update_job(job_id,status="completed",progress=100,message="Process complete",result=result)
    except Exception as exc:print(f"Job {job_id} failed: {exc}");update_job(job_id,status="failed",progress=0,message="Processing failed",error=str(exc))
    finally:input_file.unlink(missing_ok=True)

@app.get("/")
def home():return {"status":"running","service":"ClipForge AI","version":"5.0.0","format":"9:16","resolution":"1080x1920"}
@app.get("/health")
def health():return {"status":"healthy","ffmpeg":Path(FFMPEG).exists(),"ffmpeg_path":FFMPEG,"whisper":module_available("whisper"),"yt_dlp":module_available("yt_dlp"),"deno":bool(youtube_runtime_args()),"max_clips":MAX_CLIPS,"max_upload_gb":MAX_UPLOAD_BYTES/(1024**3),"features":["9:16","captions","caption styles","smart crop","blur background","audio normalization","quality presets","hook ranking","metadata","thumbnails","zip export"]}
@app.get("/jobs")
def list_jobs():
    with JOBS_LOCK:items=[{"job_id":k,**v} for k,v in JOBS.items()]
    return {"jobs":items[-20:]}
@app.post("/process")
async def process_upload(background_tasks:BackgroundTasks,file:UploadFile=File(...),num_clips:int=5,clip_duration:int=30,captions:bool=True,model:str="base",smart_crop:bool=True,generate_metadata:bool=True,language:str="auto",caption_style:str="bold",crop_mode:str="center",normalize_audio:bool=True,quality:str="high",hook_overlay:bool=False,remove_filler:bool=True):
    options=ProcessOptions(num_clips=num_clips,clip_duration=clip_duration,captions=captions,model=model,smart_crop=smart_crop,generate_metadata=generate_metadata,language=language,caption_style=caption_style,crop_mode=crop_mode,normalize_audio=normalize_audio,quality=quality,hook_overlay=hook_overlay,remove_filler=remove_filler)
    ...
    try:
        with input_file.open("wb") as buffer:
            while chunk:=await file.read(1024*1024):
                total+=len(chunk)
                if total>MAX_UPLOAD_BYTES:input_file.unlink(missing_ok=True);raise HTTPException(413,"Video is larger than 2 GB")
                buffer.write(chunk)
    finally:await file.close()
    with JOBS_LOCK:JOBS[job_id]={"status":"queued","progress":5,"message":"Queued for processing…"}
    background_tasks.add_task(run_job,job_id,input_file,options);return {"status":"queued","job_id":job_id,"status_url":f"/jobs/{job_id}"}
@app.post("/process-url")
def process_youtube(request:YouTubeRequest,background_tasks:BackgroundTasks):
    url=str(request.url)
    ...
    def task():
        try:update_job(job_id,status="processing",progress=8,message="Downloading YouTube video…");download_youtube(url,input_file,job_id);update_job(job_id,progress=30,message="Download complete. Starting AI analysis…");run_job(job_id,input_file,request.options)
        except Exception as exc:print(f"YouTube job {job_id} failed: {exc}");update_job(job_id,status="failed",progress=0,message="YouTube processing failed",error=str(exc));input_file.unlink(missing_ok=True)
    background_tasks.add_task(task);return {"status":"queued","job_id":job_id,"status_url":f"/jobs/{job_id}"}
@app.get("/jobs/{job_id}")
...
def transcript(job_id:str):
    ...
    data=json.loads(path.read_text(encoding="utf-8"));txt=OUTPUT_DIR/f"{job_id}_transcript.txt";txt.write_text(data.get("transcript", ""),encoding="utf-8");return FileResponse(txt,media_type="text/plain",filename=txt.name)
...
def download_clip(filename:str):
    ...
    suffix=candidate.suffix.lower();media="image/jpeg" if suffix==".jpg" else "application/json" if suffix==".json" else "video/mp4";return FileResponse(candidate,media_type=media,filename=candidate.name)
