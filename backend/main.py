...
for folder in (UPLOAD_DIR, OUTPUT_DIR): folder.mkdir(parents=True, exist_ok=True)
...
if not WHISPER_FFMPEG.exists():
    try: WHISPER_FFMPEG.symlink_to(FFMPEG)
    except (OSError, NotImplementedError):
        try: shutil.copy2(FFMPEG, WHISPER_FFMPEG)
        except OSError: pass
if WHISPER_FFMPEG.exists(): os.environ["PATH"] = str(LOCAL_BIN) + os.pathsep + os.environ.get("PATH", "")
...
app = FastAPI(title="ClipForge AI", version="5.2.0", description="Local AI video repurposing studio")
...
class ProcessOptions(BaseModel):
    ...
    crop_mode: str = Field("center", pattern=r"^(center|blur|fit|face)$")
...
def run_command(command: list[str], timeout: int | None = None) -> subprocess.CompletedProcess[str]:
    ...
    if result.returncode != 0: raise RuntimeError(result.stderr[-8000:] or "Command failed")
    return result

def run_ffmpeg(args: list[str]) -> subprocess.CompletedProcess[str]: return run_command([FFMPEG, *args])
...
def probe_video(path: Path) -> tuple[float, bool, int, int]:
    ...
    if not match: raise RuntimeError("Could not read video duration")
...
def transcribe(path: Path, model_name: str, language: str) -> dict[str, Any]:
    try:
        ...
        if not text: raise RuntimeError("Whisper returned an empty transcript")
...
def score_text(text: str) -> float:
    ...
    score+=min(30,sum(4 for w in HOOK_WORDS if w in unique)); score+=10 if "?" in text else 0; score+=6 if "!" in text else 0
    score+=6 if len(words)>=18 else 0; score+=5 if len(words)>=35 else 0; score+=5 if any(w.isdigit() for w in words) else 0
    score+=5 if any(w in {"you","your","we","our","they","this"} for w in unique) else 0; score-=min(15,sum(lower.count(w) for w in FILLER_WORDS)*2)
...
def build_candidates(transcription: dict[str,Any], duration: float, target:int, clip_duration:int, remove_filler:bool)->list[dict[str,Any]]:
    ...
    for i,seg in enumerate(segments):
        ...
        end=min(duration,start+window); nearby=[s for s in segments if float(s.get("end",0))>=start and float(s.get("start",0))<=end]
        text=" ".join(str(s.get("text","")).strip() for s in nearby).strip() or str(seg.get("text","")).strip(); score=score_text(text)
    ...
    candidates.sort(key=lambda x:(x["score"],len(x["text"])),reverse=True); selected=[]
    for c in candidates:
...
def fallback_candidates(duration:float,target:int,clip_duration:int)->list[dict[str,Any]]:
    ...
    length=min(float(clip_duration),duration); count=min(target,max(1,int((duration+length-1)//length))); usable=max(0.0,duration-length)
...
def make_srt(transcription:dict[str,Any],start:float,end:float,path:Path)->None:
    def ts(seconds:float)->str:
        total=max(0,int(round(seconds*1000))); h,rem=divmod(total,3600000); m,rem=divmod(rem,60000); s,ms=divmod(rem,1000); return f"{h:02d}:{m:02d}:{s:02d},{ms:03d}"
    rows=[]; n=1
    for seg in transcription.get("segments",[]):
        ...
        ls,le=max(0,s-start),min(end-start,e-start); rows.append(f"{n}\n{ts(ls)} --> {ts(max(ls+.2,le))}\n{text}\n"); n+=1
    path.write_text("\n".join(rows),encoding="utf-8")

def make_ass(transcription:dict[str,Any],start:float,end:float,path:Path,style:str)->None:
    styles={"classic":("Arial",20,180,3),"bold":("Arial",24,190,4),"minimal":("Arial",18,160,2)}; font,size,margin,outline=styles[style]
    def ats(v:float)->str:
        v=max(0,v); cs=int(round(v*100)); h=cs//360000; cs%=360000; m=cs//6000; cs%=6000; s=cs//100; c=cs%100; return f"{h}:{m:02d}:{s:02d}.{c:02d}"
    lines=["[Script Info]","ScriptType: v4.00+","PlayResX: 1080","PlayResY: 1920","[V4+ Styles]","Format: Name,Fontname,Fontsize,PrimaryColour,SecondaryColour,OutlineColour,BackColour,Bold,Italic,BorderStyle,Outline,Shadow,Alignment,MarginL,MarginR,MarginV,Encoding",f"Style: Default,{font},{size},&H00FFFFFF,&H0000FFFF,&H00000000,&H80000000,-1,0,1,{outline},1,2,60,60,{margin},1","[Events]","Format: Layer,Start,End,Style,Name,MarginL,MarginR,MarginV,Effect,Text"]
    for seg in transcription.get("segments",[]):
        ...
        if not words:
            text=str(seg.get("text","")).strip()
            if text: lines.append(f"Dialogue: 0,{ats(max(0,ss-start))},{ats(min(end-start,se-start))},Default,,0,0,0,,{text}")
            continue
        for i,w in enumerate(words):
            ws,we=float(w.get("start",ss)),float(w.get("end",se)); a=max(start,ws); b=min(end,we)
            if b<=a:continue
            text=str(w.get("word","")).strip()
            if not text:continue
            clean=text.replace("{","\\{").replace("}","\\}")
            lines.append(f"Dialogue: 0,{ats(a-start)},{ats(b-start)},Default,,0,0,0,,{{\\c&H0000FFFF&}}{clean}{{\\c&H00FFFFFF&}}")
    path.write_text("\n".join(lines)+"\n",encoding="utf-8")
...
def make_metadata(text:str,number:int)->dict[str,Any]:
    words=re.findall(r"[a-zA-Z0-9']+",text.lower()); stop={"the","and","that","this","with","from","your","have","what","when","then","they","will","about","just","into","are","you","for"}; freq:dict[str,int]={}
    for w in words:
        if len(w)>3 and w not in stop:freq[w]=freq.get(w,0)+1
    topics=[w for w,_ in sorted(freq.items(),key=lambda x:x[1],reverse=True)[:7]]; tags=["#Shorts","#ClipForgeAI"]+["#"+re.sub(r"[^a-zA-Z0-9]","",x).title() for x in topics]
    return {"title":make_title(text,number),"description":text[:500],"hashtags":list(dict.fromkeys(tags)),"keywords":topics}

def video_filter(smart_crop:bool,crop_mode:str,face_center:float|None=None)->str:
    ...
    if crop_mode=="face" and face_center is not None:
        # Keep the detected speaker near the horizontal center of the 9:16 crop.
        x=f"max(0,min(iw-iw*9/16,iw*{face_center}-iw*9/32))"
        return f"scale=-2:1920,crop=1080:1920:{x}:0"
    return "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920"

def caption_style(name:str)->str:
    styles={"classic":"FontName=Arial,FontSize=20,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=1,Outline=3,Shadow=1,Alignment=2,MarginV=180","bold":"FontName=Arial,FontSize=24,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=1,Outline=4,Shadow=1,Alignment=2,MarginV=190,Bold=1","minimal":"FontName=Arial,FontSize=18,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=1,Outline=2,Alignment=2,MarginV=160}; return styles[name]

def create_thumbnail(input_file:Path,output_file:Path,start:float,smart_crop:bool,crop_mode:str,face_center:float|None=None)->None:
    run_ffmpeg(["-y","-ss",f"{start:.3f}","-i",str(input_file),"-frames:v","1","-vf",video_filter(smart_crop,crop_mode,face_center),str(output_file)])

def create_short(input_file:Path,output_file:Path,start:float,duration:float,transcription:dict[str,Any],options:ProcessOptions,face_center:float|None=None)->None:
    temp_paths=[]
    try:
        filters=[video_filter(options.smart_crop,options.crop_mode,face_center)]
        if options.captions and transcription.get("segments"):
            if options.dynamic_captions:
                with tempfile.NamedTemporaryFile(suffix=".ass",delete=False) as tmp: ass_path=Path(tmp.name)
                temp_paths.append(ass_path); make_ass(transcription,start,start+duration,ass_path); sub=str(ass_path).replace("\\","/").replace(":","\\:"); filters.append("ass="+sub)
            else:
                with tempfile.NamedTemporaryFile(suffix=".srt",delete=False) as tmp: srt_path=Path(tmp.name)
                temp_paths.append(srt_path); make_srt(transcription,start,start+duration,srt_path); sub=str(srt_path).replace("\\","/").replace(":","\\:"); filters.append("subtitles="+sub+":force_style='"+caption_style(options.caption_style)+"'")
...
def download_youtube(url:str,destination:Path,job_id:str)->None:
    ...
    template=str(destination.with_suffix(""))+".%(ext)s"; runtime=youtube_runtime_args(); selectors=["bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]","bestvideo[height<=720]+bestaudio/best[height<=720]","best[height<=1080]/best"]; last_error="No compatible YouTube format was found."
    for selector in selectors:
        try:
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:info=ydl.extract_info(url,download=True); prepared=Path(ydl.prepare_filename(info))
            candidates=[destination,prepared,prepared.with_suffix(".mp4"),prepared.with_suffix(".mkv"),prepared.with_suffix(".webm"),prepared.with_suffix(".m4v")]+list(destination.parent.glob(destination.stem+".*")); candidates=[p for p in candidates if p.is_file() and p.suffix.lower() in ALLOWED_EXTENSIONS]
            ...
            return
        except Exception as exc:last_error=str(exc); update_job(job_id,message="Trying a compatible YouTube format…")
...
def process_file(input_file:Path,job_id:str,options:ProcessOptions)->dict[str,Any]:
    ...
    face_center=None
    if options.smart_crop and options.crop_mode=="face":
        update_job(job_id,progress=66,message="Detecting speaker position locally…")
        try:
            from face_framing import detect_speaker_center
            face_center=detect_speaker_center(input_file,FFMPEG)
        except Exception as exc:print(f"Face framing unavailable: {exc}")
    ...
    for index,h in enumerate(highlights,1):
        ...
        create_short(input_file,output,start,actual,transcription,options,face_center); thumb=OUTPUT_DIR/f"{job_id}_clip_{index}.jpg"
        try:create_thumbnail(input_file,thumb,start,options.smart_crop,options.crop_mode,face_center)
        except Exception:thumb=None
        metadata=make_metadata(h["text"],index) if options.generate_metadata else {}
        clips.append({"clip_number":index,"title":metadata.get("title",f"AI Short #{index}"),"score":h["score"],"text":h["text"],"start":round(start,2),"end":round(end,2),"duration":round(actual,2),"download_url":f"/download/{output.name}","thumbnail_url":f"/download/{thumb.name}" if thumb else None,"metadata":metadata,"format":"9:16","resolution":"1080x1920","framing":"face-aware" if face_center is not None and options.crop_mode=="face" else options.crop_mode})
        update_job(job_id,progress=min(95,70+int(index/max(1,len(highlights))*25)),message=f"Rendered clip {index}/{len(highlights)}")
    manifest={"job_id":job_id,"source":input_file.name,"source_duration":duration,"source_resolution":f"{width}x{height}","output_format":"9:16","output_resolution":"1080x1920","whisper":transcription.get("language","unknown"),"transcription_error":transcription.get("error"),"settings":options.model_dump(),"face_center":face_center,"clips":clips}
    manifest_path=OUTPUT_DIR/f"{job_id}_manifest.json";manifest_path.write_text(json.dumps(manifest,indent=2,ensure_ascii=False),encoding="utf-8");zip_path=OUTPUT_DIR/f"{job_id}_shorts.zip"
...
def run_job(job_id:str,input_file:Path,options:ProcessOptions)->None:
    try:result=process_file(input_file,job_id,options);update_job(job_id,status="completed",progress=100,message="Done",result=result)
    except Exception as exc:print(f"Job {job_id} failed: {exc}");update_job(job_id,status="failed",progress=100,message=str(exc),error=str(exc))
    finally:input_file.unlink(missing_ok=True)

@app.get("/")
def root():return {"name":"ClipForge AI","version":"5.2.0","format":"9:16","resolution":"1080x1920","whisper_ffmpeg":WHISPER_FFMPEG.exists()}

@app.get("/health")
def health():return {"status":"ok","version":"5.2.0","ffmpeg":Path(FFMPEG).name,"whisper_ffmpeg":WHISPER_FFMPEG.exists(),"whisper":module_available("whisper"),"opencv":module_available("cv2"),"features":["local-whisper","hook-ranking","filler-reduction","captions","dynamic-word-captions","face-aware-framing","9:16","1080x1920","blur-background","audio-normalization","quality-presets","metadata","thumbnails","zip-export"]}
...
async def process(background_tasks:BackgroundTasks,file:UploadFile=File(...),options:str="{}"):
    ...
    background_tasks.add_task(run_job,job_id,destination,opts);return {"job_id":job_id}
...
async def process_url(request:YouTubeRequest,background_tasks:BackgroundTasks):
    ...
    def download_and_run():
        try:download_youtube(url,destination,job_id);update_job(job_id,status="processing",progress=30,message="Download complete");run_job(job_id,destination,request.options)
        except Exception as exc:print(f"YouTube job {job_id} failed: {exc}");update_job(job_id,status="failed",progress=100,message=str(exc),error=str(exc));destination.unlink(missing_ok=True)
    background_tasks.add_task(download_and_run);return {"job_id":job_id}
